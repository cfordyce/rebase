...
StringFEPVHMap::StringFEPVHMap(FEP  dflt, unsigned int sz)
     :StringFEPMap(dflt)
{
...
StringFEPVHMap::StringFEPVHMap(StringFEPVHMap& a) : StringFEPMap(a.def)
{
...
Pix StringFEPVHMap::seek(String& key)
{
...
FEP& StringFEPVHMap::operator [](String& item)
{
...
void StringFEPVHMap::del(String& key)
{
  unsigned int hashval = StringHASH(key);
  unsigned int h = hashval % size;
  for (unsigned int i = 0; i <= size; ++i)
...
void StringFEPVHMap::clear()
{
  for (unsigned int i = 0; i < size; ++i) status[i] = EMPTYCELL;
  count = 0;
}

void StringFEPVHMap::resize(unsigned int newsize)
{
  if (newsize <= count)
  {
...
void StringFEPVHMap::next(Pix& i)
{
  if (i == 0) return;
