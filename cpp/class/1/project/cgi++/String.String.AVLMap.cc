...
static inline int bf(StringStringAVLNode* t)
{
  return t->stat & AVLBALANCEMASK;
}

static inline void set_bf(StringStringAVLNode* t, int b)
{
  t->stat = (t->stat & ~AVLBALANCEMASK) | (b & AVLBALANCEMASK);
}


static inline int rthread(StringStringAVLNode* t)
{
  return t->stat & RTHREADBIT;
}

static inline void set_rthread(StringStringAVLNode* t, int b)
{
  if (b)
    t->stat |= RTHREADBIT;
  else
...
static inline int lthread(StringStringAVLNode* t)
{
  return t->stat & LTHREADBIT;
}

static inline void set_lthread(StringStringAVLNode* t, int b)
{
  if (b)
    t->stat |= LTHREADBIT;
  else
...
StringStringAVLNode* StringStringAVLMap::leftmost()
{
  StringStringAVLNode* t = root;
  if (t != 0) while (t->lt != 0) t = t->lt;
  return t;
}

StringStringAVLNode* StringStringAVLMap::rightmost()
{
  StringStringAVLNode* t = root;
...
StringStringAVLNode* StringStringAVLMap::succ(StringStringAVLNode* t)
{
  StringStringAVLNode* r = t->rt;
  if (!rthread(t)) while (!lthread(r)) r = r->lt;
  return r;
}

StringStringAVLNode* StringStringAVLMap::pred(StringStringAVLNode* t)
{
  StringStringAVLNode* l = t->lt;
  if (!lthread(t)) while (!rthread(l)) l = l->rt;
  return l;
}


Pix StringStringAVLMap::seek(String& key)
{
  StringStringAVLNode* t = root;
  if (t == 0)
    return 0;
...
static StringStringAVLNode* _found_node; // returned added/deleted node
static int    _already_found;   // for deletion subcases


void StringStringAVLMap:: _add(StringStringAVLNode*& t)
{
  ...
  else if (cmp < 0)
  {
    ...
    {
      ++count;
      _found_node = new StringStringAVLNode(*_target_item, def);
...
String& StringStringAVLMap::operator [] (String& item)
{
  if (root == 0)
  {
    ++count;
    root = new StringStringAVLNode(item, def);
    set_rthread(root, 1);
    set_lthread(root, 1);
    return root->cont;
  }
...
void StringStringAVLMap::_del(StringStringAVLNode* par, StringStringAVLNode*& t)
{
  int comp;
  if (_already_found)
  {
...
void StringStringAVLMap::del(String& item)
{
  if (root == 0) return;
  _need_rebalancing = 0;
  _already_found = 0;
...
void StringStringAVLMap::_kill(StringStringAVLNode* t)
{
  if (t != 0)
  {
...
StringStringAVLMap::StringStringAVLMap(StringStringAVLMap& b) :StringStringMap(b.def)
{
  root = 0;
  count = 0;
...
int StringStringAVLMap::OK()
{
  ...
  {
    int n = 1;
    StringStringAVLNode* trail = leftmost();
    StringStringAVLNode* t = succ(trail);
    while (t != 0)
    {
      ++n;
